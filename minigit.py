...
class Repository:
    ...
    @staticmethod
    def clone_repo(source_path: str, dest_path: str):
        source = Path(source_path).resolve()
        dest = Path(dest_path).resolve()

        src_git = source / ".git"
        if not src_git.exists():
            raise Exception("Source is not a git repository (.git folder missing)")

        # create destination directory if needed
        dest.mkdir(parents=True, exist_ok=True)

        # copy .git
        shutil.copytree(src_git, dest / ".git", dirs_exist_ok=True)

        print(f"Cloned repository from {source} to {dest}")

        # reconstruct working files from the HEAD commit
        cloned_repo = Repository(dest)
        branch = cloned_repo.get_current_branch()
        cloned_repo.restore_working_directory(branch, set())

        print("Working directory restored.")

    def push(self, dest_path: str):
        dest_repo = Repository(dest_path)
        if not dest_repo.git_dir.exists():
            raise Exception("Destination is not a git repository")

        for obj_dir in self.objects_dir.iterdir():
            if obj_dir.is_dir():
                for obj_file in obj_dir.iterdir():
                    src_obj = obj_file
                    dest_obj = dest_repo.objects_dir / obj_dir.name / obj_file.name
                    if not dest_obj.exists():
                        dest_obj.parent.mkdir(parents=True, exist_ok=True)
                        shutil.copy2(src_obj, dest_obj)

        print(f"Pushed objects to {dest_path}")
    ...
    def pull(self, src: str):
        ...
        # Local directory pull (existing logic)
        src_repo = Repository(src)
        if not src_repo.git_dir.exists():
            raise Exception("Source is not a git repository")

        # copy missing objects (local sync)
        for obj_dir in src_repo.objects_dir.iterdir():
            if obj_dir.is_dir():
                for obj_file in obj_dir.iterdir():
                    dest_obj = self.objects_dir / obj_dir.name / obj_file.name
                    if not dest_obj.exists():
                        dest_obj.parent.mkdir(parents=True, exist_ok=True)
        ...
        # sync branch pointer
        current_branch = self.get_current_branch()
        src_commit = src_repo.get_branch_commit(current_branch)
        if src_commit:
            self.set_branch_commit(current_branch, src_commit)

        print(f"Pulled objects from {src}")

    def pull_http(self, base_url: str):
        
        print(f"Pulling via HTTP from {base_url}")

        for prefix in range(256):
            prefix_hex = f"{prefix:02x}"
            url_prefix = f"{base_url}/objects/{prefix_hex}/"

            try:
                index_page = requests.get(url_prefix).text
            except:
                continue

            # Extract SHA suffix filenames from HTML directory listing
            for suffix in re.findall(r'href="([0-9a-f]{38})"', index_page):
                full_hash = prefix_hex + suffix
                dest_dir = self.objects_dir / full_hash[:2]
                dest_file = dest_dir / full_hash[2:]

                if dest_file.exists():
                    continue

                dest_dir.mkdir(parents=True, exist_ok=True)
                obj_url = f"{base_url}/objects/{prefix_hex}/{suffix}"
                obj_data = requests.get(obj_url).content
                dest_file.write_bytes(obj_data)

        print("HTTP object sync complete.")

        # ✅ Sync branch pointer
        branch = self.get_current_branch()
        remote_head_url = f"{base_url}/HEAD"
        head_content = requests.get(remote_head_url).text.strip()

        if head_content.startswith("ref:"):
            ref_path = head_content.split(" ")[1]   # refs/heads/master
            remote_ref_url = f"{base_url}/{ref_path}"
            commit_hash = requests.get(remote_ref_url).text.strip()
            if commit_hash:
                self.set_branch_commit(branch, commit_hash)
                print(f"Updated branch '{branch}' to commit {commit_hash}")





def announce_peer(port=MESHGIT_PORT):
    message = b"MESHGIT_PEER"
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    sock.setsockopt(socket.SOL_SOCKET, socket.SO_BROADCAST, 1)

    print("Broadcasting presence… (Ctrl+C to stop)")
    while True:
        sock.sendto(message, ('<broadcast>', port))
        time.sleep(2)


def listen_for_peers(port=MESHGIT_PORT):
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)   # <-- Required on Windows
    sock.bind(('', port))

    print(f"Listening for MeshGit peers on port {port}…")
    while True:
        data, addr = sock.recvfrom(1024)
        if data == b"MESHGIT_PEER":
            print(f"🔥 Peer detected at {addr[0]}")

def start_mesh_server(port=8080):
    repo = Repository()
    server_path = repo.git_dir  # serve .git directory directly
    print(f"Serving objects from {server_path}")

    class MeshHandler(SimpleHTTPRequestHandler):
        def translate_path(self, path):
            return str(server_path / path.lstrip("/"))

    server = HTTPServer(("0.0.0.0", port), MeshHandler)
    print(f"🌐 MeshGit sync server running at http://0.0.0.0:{port}")
...
def main():
    ...
    clone_parser.add_argument("source", help="Path to source repository")
    clone_parser.add_argument("destination", help="Path to clone into")

    # status command
    status_parser = subparsers.add_parser("status", help="Show repository status")

    # push
    push_parser = subparsers.add_parser("push", help="Push objects to another repo")
    push_parser.add_argument("destination", help="Path to target repository")

    # pull
    pull_parser = subparsers.add_parser("pull", help="Pull objects from another repo")
    pull_parser.add_argument("source", help="Path to source repository")

    
    peer_parser = subparsers.add_parser("peers", help="Discover nearby MeshGit peers")
    peer_parser.add_argument("--listen", action="store_true", help="Listen for peers")


    meshserver_parser = subparsers.add_parser("meshserver", help="Start MeshGit sync server")
    meshserver_parser.add_argument("--port", type=int, default=8080)
    ...
    try:
        if args.command == "init":
            ...
            Repository.clone_repo(args.source, args.destination)
            return
        
        elif args.command == "push":
            repo.push(args.destination)

        elif args.command == "pull":
            repo.pull(args.source)
        ...
        elif args.command == "meshserver":
            start_mesh_server(args.port)
